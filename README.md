...
Basis Universal is not an image compression codec. It's a texture data compression codec. It can be used just like an image compression codec, but that's not the only use case. It's one of the first systems to support all of the LDR GPU texture formats in a unified way. Here's a [good intro](http://renderingpipeline.com/2012/07/texture-compression/) to texture compression.
